...
void main() {
	...
	//frag_color = texture2D(lumtex, uv); //vec4(vec3(mcg), mc.a); //vec4(vec3(lum(mc)), mc.a);
	//return;
	/*if(mcg == 1.0) {  
		frag_color = mc;
		return;
	...
	// Kernel 0 and 4
	if (min3(tlg, tcg, trg) > max4(mcg, brg, bcg, blg)) frag_color = average(mc, tl, tc, tr);
	else if (min3(brg, bcg, blg) > max4(mcg, tlg, tcg, trg)) frag_color = average(mc, br, bc, bl);

	// Kernel 1 and 5
	else if (min3(mrg, tcg, trg) > max3(mcg, mlg, bcg	)) frag_color = average(mc, mr, tc, tr);
	else if (min3(blg, mlg, bcg) > max3(mcg, mrg, tcg	)) frag_color = average(mc, bl, ml, bc);

	// Kernel 2 and 6
	else if (min3(mrg, brg, trg) > max4(mcg, mlg, tlg, blg)) frag_color = average(mc, mr, br, tr);
	else if (min3(mlg, tlg, blg) > max4(mcg, mrg, brg, trg)) frag_color = average(mc, ml, tl, bl);

	// Kernel 3 and 7
	else if (min3(mrg, brg, bcg) > max3(mcg, mlg, tcg	)) frag_color = average(mc, mr, br, bc);
	else if (min3(tcg, mlg, tlg) > max3(mcg, mrg, bcg	)) frag_color = average(mc, tc, ml, tl);
