...
// KID image format, used before Remember11
...
#ifdef LOW_RES
#define IS_LOW_RES true
#else
#define IS_LOW_RES false
#endif

u8* load_cps(u8* data, int size, int* width, int* height, int* out_fmt) {
	u32 unpacked_size;
	data = prepare_cps(data, size, &unpacked_size);
	u32 sig = U32(data);
	u16 version = U16LE(data + 4);
	if(version != 101 && version != 102) {
		log_err("unsupported prt version %u", version);
		platform_throw();
	}
	u16 img_w = U16LE(data + 0xC);
	u16 img_h = U16LE(data + 0xE);
	u16 bpp = U16LE(data + 6);
	//log_info("%u bpp", bpp);
	bool has_alpha = U32LE(data + 0x10);
	*out_fmt = TEX_FORMAT_RGBA;
	version = version;
	...
	bool half_res = IS_LOW_RES && img_w > 544 && img_h > 544;
	u32 w, h;
	if(half_res) {
		w = img_w / 2;
		h = img_h / 2;
	...
	u32* out = new u32[w * h];
	u32 stride = (img_w * (bpp / 8) + 3) & ~3;
	int fmt;
	*width = w;
	*height = h;
	if(bpp == 8) {
		u32 j = 0;
		for(u32 y = 0; y < img_h; y += half_res? 2: 1) {
			for(u32 x = 0; x < img_w; x += half_res? 2: 1, j++) {
				out[j] = ((u32*) (data + palette_off))[U8(data + data_off + (img_h - y - 1) * img_w + x)] | 0xFF000000;
				out[j] = (out[j] & 0xFF000000) >> 0 | (out[j] & 0xFF0000) >> 16 | (out[j] & 0xFF00) << 0 | (out[j] & 0xFF) << 16;
			}
		}
	} else if(bpp == 24) {
		u8* alpha = data + data_off + stride * img_h;
		u32 j = 0;
		for(u32 yi = data_off + stride * img_h - stride; yi >= data_off && j < w * h; yi -= stride) {
			for(u32 i = 0; i < img_w * 3; i += 3, j++) {
				//u32 c = U32LE(data + yi + i) & 0xFFFFFF;
				if(half_res) {
					u8 a = ((u32) U8(alpha) + U8(alpha + 1) + U8(alpha + img_w) + U8(alpha + img_w + 1)) / 4;
...
//#endif
					...
					u8 g = U8(data + yi + i + 1);
					u8 b = U8(data + yi + i);
					out[j] = (u32) (has_alpha? U8(alpha++) << 24: 0xFF000000) | (u32) b << 16 | (u32) g << 8 | (u32) r;
				}
			}
			if(half_res) {
				yi -= stride;
