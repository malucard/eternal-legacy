...
local pc_map = {
	...
	[bit.bor(INPUT_UP, INPUT_DOWN)] = "res/buttons/dpadud.png",
	[INPUT_LEFT] = "res/buttons/dpadl.png",
	[INPUT_RIGHT] = "res/buttons/dpadr.png",
	[bit.bor(INPUT_LEFT, INPUT_RIGHT)] = "res/buttons/dpadlr.png",
	[bit.bor(bit.bor(INPUT_UP, INPUT_DOWN), bit.bor(INPUT_LEFT, INPUT_RIGHT))] = "res/buttons/dpad.png"
}
...
local halo_settings = {halo_alpha = 0.75, halo_lightness = 0, halo_detail = nil, halo_distance = nil, shadow_distance = 2.5}
function bh_draw(inputs)
	local x = 16
	for _, inp in ipairs(inputs) do
		if type(inp) == "string" then
			local textinfo = g_draw_text(inp, x, -22, -1, 24, halo_settings, G_BOTTOM_LEFT, G_BOTTOM_LEFT)
			x = x + textinfo.w + 16
		else
